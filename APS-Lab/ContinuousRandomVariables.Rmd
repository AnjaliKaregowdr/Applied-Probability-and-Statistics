...
Poisson random variable explaination
```{r}
n=10
p=0.2
rbinom(100, n, p)
...
x=seq(138,202,0.1)
f_X=function(x){
  return(exp(-(x-170)^2/(2*(8^2)))/sqrt(2*pi*64))    #170 and 8 are mean and standard deviation
}
f_Y=function(x){
  return(exp(-(x-190)^2/(2*(4^2)))/sqrt(2*pi*16))
}
#y1=f_X(x)
y1=dnorm(x,170,8)
#y2=f_Y(x)
y2=dnorm(x,170,4)    #probability density
```
...
x=165
dnorm(x,mu,sigma)
pnorm(x,mu,sigma)   #height is atmost x cm
pnorm(173, mu, sigma) - pnorm(169,mu,sigma)  #height in between 169 and 173
pnorm(173,mu,sigma, lower.tail = FALSE)
```
...
mu_Male = 170
sigma_Male = 8
#http://127.0.0.1:26113/graphics/cd100cf8-df62-4f82-80e6-48cc475c1d23.png
# Bin width
delta = 2.0
...
```{r}
sum(simulatedData_Male/nsimulations) #mean
mean((simulatedData_Male - mean(simulatedData_Male))^2) #squared deviations  -> On an avg how much squared deviation this  
sqrt(mean((simulatedData_Male - mean(simulatedData_Male))^2))   ->  # deviation -> On an avg how much deviation this
```
